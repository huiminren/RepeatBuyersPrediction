...
def main(args: Array[String]){
	if (args.length < 5) {
      System.err.println("Usage: RepeatBuyers <logInput> <infoInput>" +
        "<trainInput> <testInput>" + "<outputPath> [<filters>]")
      System.exit(1)
    }
    val Array(logInput, infoInput, trainInput, testInput, outputPath) = args.take(5)
